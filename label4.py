...
# %%
cross_val_score(SVC(C=100, gamma=0.001, class_weight='balanced'), standardized_train_features, train_label4, cv=5, verbose=3).mean()

# %%
best_model_label_4 = SVC(C=100, gamma=0.001, class_weight='balanced')
pred_label4 = best_model_label_4.fit( standardized_train_features, train_label4).predict( standardized_test_features)
pred = best_model_label_4.predict( standardized_valid_features)
...
def create_csv(pred):
  ...
  df.to_csv('/kaggle/working/190676J_l12_label_4.csv', index=False)
